...
window.onload = ->
  ...
  $_("#join").onclick = ->
    ...
    video.addEventListener "timeupdate", (->
      conn.send video.currentTime  if iAmControlling()
      return
    ), true
    video.addEventListener "pause", (->
      conn.send "pause " + video.currentTime  if iAmControlling()
      return
    ), true
    video.addEventListener "play", (->
      conn.send "play " + video.currentTime  if iAmControlling()
      return
    ), true
    conn.onmessage = (ev) ->
      matches = undefined
      if matches = ev.data.match(/^control (.+)$/)
        ...
        estimatedTimeOnMaster = parseInt(ev.data) + 1
    ...
    $_("#like").onclick = ->
      conn.send "like " + video.currentTime
      return
